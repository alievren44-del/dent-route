...
const PARLA_TABLE_ALTER_ALLOWLIST = new Set<string>([
...
const PARLA_TABLE_INSERT_ALLOWLIST = new Set<string>([
...
const LOCAL_ONLY_FILES = new Set<string>([
...
function checkFilename(file: string): void {
...
function extractTablesFromStatement(stmt: string): { action: string; table: string; line: number }[] {
  const found: { action: string; table: string; line: number }[] = [];

  // Statement boundary öncesi: ; \n veya başlangıç. \b kelime sınırı statement start için yetersiz.
  const stmtBoundary = `(?:^|;|\\nBEGIN\\b|EXECUTE\\s+'|[\\n\\r]\\s*)`;

  const patterns: { action: string; re: RegExp }[] = [
  ...
  for (const { action, re } of patterns) {
    ...
    while ((m = re.exec(stmt)) !== null) {
      ...
      if (table) found.push({ action, table, line: 0 });
    }
  }
...
function checkSqlContent(file: string, sql: string): void {
  ...
  for (const ref of refs) {
    ...
    if (table.startsWith('saha_')) continue;

    const isAllowedAlter =
      (ref.action === 'ALTER TABLE' || ref.action === 'UPDATE') &&
      PARLA_TABLE_ALTER_ALLOWLIST.has(table);

    const isAllowedSeed =
      (ref.action === 'INSERT INTO' || ref.action === 'DELETE FROM' || ref.action === 'UPDATE') &&
      PARLA_TABLE_INSERT_ALLOWLIST.has(table);

    if (isAllowedAlter || isAllowedSeed) continue;
  ...
  while ((fm = fnRe.exec(cleaned)) !== null) {
    ...
    if (!name) continue;
    if (name.startsWith('saha_') || name === 'set_updated_at') continue;
