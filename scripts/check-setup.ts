...
function loadEnv(): void {
  ...
  const procWithLoad = process as typeof process & {
...
function looksLikeSupabaseKey(value: string): boolean {
  // Yeni Supabase API key formatı (`sb_publishable_...`, `sb_secret_...`)
  // veya eski JWT formatı.
...
async function main(): Promise<void> {
  ...
  } else if (!looksLikeSupabaseKey(anonKey)) {
    fail('VITE_SUPABASE_ANON_KEY formatında değil (sb_publishable_... veya JWT bekleniyor)');
  } else {
  ...
  } else if (!looksLikeSupabaseKey(serviceRoleKey)) {
    fail('SUPABASE_SERVICE_ROLE_KEY formatında değil (sb_secret_... veya JWT bekleniyor)');
  } else {
