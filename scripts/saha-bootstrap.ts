...
async function main() {
  ...
  if (!existsSync(configPath)) {
    ...
    log.info(
      `Çözüm: cp config/.saha-config.example.json config/.saha-config.json && tenant için düzenle`,
    );
    process.exit(1);
  }
  ...
  let vertical: Record<string, any>;
  try {
  ...
  log.ok(
    `${vertical.customerTypes?.length ?? 0} müşteri tipi · ${vertical.visitOutcomes?.length ?? 0} ziyaret outcome · ${(vertical.customFields?.account?.length ?? 0) + (vertical.customFields?.visit?.length ?? 0)} custom field`,
  );
