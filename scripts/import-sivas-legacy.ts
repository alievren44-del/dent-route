...
import {
  ...
  validateRows,
  type ClinicRow,
} from '../src/features/admin/lib/clinicXlsxParser';
...
if (!SUPABASE_URL || !SERVICE_KEY) {
  console.error(
    'SUPABASE_URL (veya VITE_SUPABASE_URL) ve SUPABASE_SERVICE_ROLE_KEY env eksik.',
  );
  process.exit(1);
}
...
const FILE_PATH = resolve(
  process.cwd(),
  'data-legacy',
  'Sivas_GENELI_Birlesik_Dis_Hekimi_Rotasi_v3.xlsx',
);
...
async function main(): Promise<void> {
  ...
  if (sheetsToProcess.length === 0) {
    console.error(
      `İşlenecek sheet bulunamadı. Beklenenler: ${PROCESS_SHEETS.join(', ')}`,
    );
    process.exit(1);
  }
  ...
  for (const [name, s] of Object.entries(bySheet)) {
    console.log(
      `  ${name.padEnd(20)} processed=${s.processed} created=${s.created} updated=${s.updated} skipped=${s.skipped}`,
    );
  }
}
