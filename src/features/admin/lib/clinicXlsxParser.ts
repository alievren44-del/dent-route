...
export interface ValidationResult {
  valid: ClinicRow[];
  skipped: Array<{ row: ClinicRow; reason: string }>;
}
 ...
 */
export function parseSheetNames(arrayBuffer: ArrayBuffer): string[] {
...
export function inferColumnMapping(
  ...
  for (const rule of rules) {
    ...
    if (found) mapping[rule.target] = found;
  }
...
export function inferClinicTypeFromSheet(sheetName: string): ClinicType {
...
export function mapRows(
  ...
  for (const r of rows) {
    ...
    if (latCol) {
      ...
      if (v !== undefined) row.lat = v;
    }
    if (lngCol) {
      const v = toNum(r[lngCol]);
      if (v !== undefined) row.lng = v;
    }
    if (nbhCol) {
      const v = toStr(r[nbhCol]);
      if (v) row.neighborhood = v;
    }
    if (inferredType) {
 ...
 */
export function validateRows(rows: ClinicRow[]): ValidationResult {
  const valid: ClinicRow[] = [];
  const skipped: Array<{ row: ClinicRow; reason: string }> = [];
 ...
 */
export function syntheticPlaceId(row: ClinicRow, prefix = 'legacy_sivas'): string {
